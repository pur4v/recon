...
`skills/recon/scripts/decode.mjs` where the app ships one. Fan out one sub-agent per
section. Count honestly (unique features vs. modes; exclude coming-soon/stubs) and attach a
locator to every claim. Run the adversarial verify pass before presenting.
